...
async def run_stage3(base_dir: Path, config: StudyConfig) -> None:
    ...
    async with (
        ...
        for i, login in enumerate(author_logins):
            if author_already_fetched(authors_dir, login):
                continue

            logger.info(
                "Fetching author %d/%d: %s",
                i + 1, len(author_logins), login,
            )

            try:
                # Fetch contribution data via GE
                ...
                # Tier 1: PR counts by state
                tier1 = await _fetch_tier1_counts(
                    http_client, token, login,
                )
                record.update(tier1)

                # Tier 2: detailed closed PR analysis
                ...
                logger.exception(
                    "Failed to fetch data for %s", login,
                )
                continue
